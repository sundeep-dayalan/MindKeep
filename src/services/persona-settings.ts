...
import type { Persona, PersonaSettings } from "~types/persona"
...
export async function getPersonaSettings(): Promise<PersonaSettings> {
  console.log("⚙️ [Settings] getPersonaSettings called")
  ...
  try {
    ...
    console.log("⚙️ [Settings] No settings found, using defaults")
    return DEFAULT_SETTINGS
  } catch (error) {
    console.error("⚙️ [Settings] Error getting settings:", error)
    return DEFAULT_SETTINGS
  }
}
...
export async function updatePersonaSettings(
  ...
  console.log("⚙️ [Settings] updatePersonaSettings called with:", updates)
  ...
  try {
    ...
    console.log("⚙️ [Settings] Settings updated:", newSettings)
    
    return newSettings
  } catch (error) {
    console.error("⚙️ [Settings] Error updating settings:", error)
    throw error
  }
}
...
export async function setSelectedPersona(
  ...
  console.log("⚙️ [Settings] setSelectedPersona called with ID:", personaId)
...
export async function getSelectedPersonaId(): Promise<string | null> {
  console.log("⚙️ [Settings] getSelectedPersonaId called")
  
  const settings = await getPersonaSettings()
  console.log("⚙️ [Settings] Selected persona ID:", settings.selectedPersonaId)
...
export async function setDefaultPersona(
  ...
  console.log("⚙️ [Settings] setDefaultPersona called with ID:", personaId)
...
export async function clearPersonaSettings(): Promise<PersonaSettings> {
  console.log("⚙️ [Settings] clearPersonaSettings called")
  
  try {
    await chrome.storage.local.remove(SETTINGS_KEY)
    console.log("⚙️ [Settings] Settings cleared")
    
    return DEFAULT_SETTINGS
  } catch (error) {
    console.error("⚙️ [Settings] Error clearing settings:", error)
    throw error
  }
}
...
export function onPersonaSettingsChanged(
  ...
  console.log("⚙️ [Settings] Setting up settings change listener")
  ...
  const listener = (
    ...
    if (areaName === "local" && changes[SETTINGS_KEY]) {
      ...
      console.log("⚙️ [Settings] Settings changed:", newSettings)
  ...
  return () => {
    console.log("⚙️ [Settings] Removing settings change listener")
